...
function createSetting(&$modx,$key,$value) {
    $ct = $modx->getCount('modSystemSetting',array(
        'key' => 'themebootstrap.'.$key,
    ));
    if (empty($ct)) {
        $setting = $modx->newObject('modSystemSetting');
        $setting->set('key','themebootstrap.'.$key);
        $setting->set('value',$value);
        $setting->set('namespace','themebootstrap');
...
if ($object->xpdo) {
    switch ($options[xPDOTransport::PACKAGE_ACTION]) {
        ...
        case xPDOTransport::ACTION_UPGRADE:
            ...
            /* setup paths */
            createSetting($modx,'core_path',$modx->getOption('core_path').'components/themebootstrap/');
            createSetting($modx,'assets_path',$modx->getOption('assets_path').'components/themebootstrap/');

            /* setup urls */
            createSetting($modx,'assets_url',$modx->getOption('assets_url').'components/themebootstrap/');
        break;
    }
}
return true;